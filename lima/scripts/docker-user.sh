...
#
# No systemd drop-in is installed: with the slirp4netns package present,
# dockerd-rootless.sh already defaults to --net=slirp4netns --port-driver=builtin,
# the pair that can publish on the guest's own vzNAT IP. See docker-system.sh.
set -eux
...
# which linger keeps running across logout and reboots.
