...
set -l dev_vm_commands create recreate start stop destroy list status completion version help

complete -c dev-vm -f

complete -c dev-vm -n "not __fish_seen_subcommand_from $dev_vm_commands" -a create -d 'create a dev VM'
complete -c dev-vm -n "not __fish_seen_subcommand_from $dev_vm_commands" -a recreate -d 'rebuild a dev VM, keeping the guest home directory'
...
complete -c dev-vm -n '__fish_seen_subcommand_from recreate' -o create-ssh-key -r -a 'true false' -d 'create and register a new key'
complete -c dev-vm -n '__fish_seen_subcommand_from recreate' -o force -d 'skip the confirmation prompt'
complete -c dev-vm -n '__fish_seen_subcommand_from recreate' -o cpus -r -d 'vCPUs for the VM'
complete -c dev-vm -n '__fish_seen_subcommand_from recreate' -o memory -r -d 'RAM in GiB'
complete -c dev-vm -n '__fish_seen_subcommand_from recreate' -o disk -r -d 'disk size in GiB'

complete -c dev-vm -n '__fish_seen_subcommand_from recreate start stop destroy status' -a '(__dev_vm_names)' -d 'dev VM'
