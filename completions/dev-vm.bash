...
_dev_vm() {
    ...
    if [ "$COMP_CWORD" -eq 1 ]; then
        COMPREPLY=($(compgen -W "create recreate start stop destroy list status completion version help" -- "$cur"))
        return
    fi
    ...
    recreate)
        ...
        if [ "${cur:0:1}" = "-" ]; then
            COMPREPLY=($(compgen -W "-create-ssh-key= -force -cpus -memory -disk -help" -- "$cur"))
        else
